...
comptime CtxPtr = UnsafePointer[DeviceContext, AnyOrigin[mut=True]]
...
def mpmc_create_cpu_context() abi("C") -> Int:
    try:
        var ctx = alloc[DeviceContext](1).as_unsafe_any_origin()
        ctx.unsafe_write(DeviceContext(api="cpu"))
        return Int(ctx)
    except:
        return 0
...
def mpmc_destroy_cpu_context(ctx_addr: Int) abi("C"):
    var ctx = CtxPtr(unsafe_from_address=ctx_addr)
    _ = ctx.take_pointee()
    ctx.free()
...
def diag_velocity_energy_parallel(
    ...
    var chunk_size = ((n + ELEMENT_WORKERS - 1) // ELEMENT_WORKERS + W - 1) // W * W
    ...
    parallelize[compute_chunk](ELEMENT_WORKERS, ELEMENT_WORKERS, ctx)
    var energy = 0.0
...
def leapfrog_first_diag(
    q: Ptr, momentum: Ptr, grad: Ptr, diag: Ptr, velocity: Ptr,
...
def leapfrog_first_full(
    q: Ptr, momentum: Ptr, grad: Ptr, matrix: Ptr, velocity: Ptr,
    epsilon: Float64, n: Int
):
...
def leapfrog_first_inv(
    q: Ptr, momentum: Ptr, grad: Ptr, chol: Ptr, velocity: Ptr,
    epsilon: Float64, n: Int
):
    add_scaled(momentum, grad, 0.5 * epsilon, n)
...
def exp_var_add(
    value: Ptr, mean: Ptr, variance: Ptr, alpha: Float64, n: Int
):
    for i in range(n):
...
def welford_cov_add(
    value: Ptr, mean: Ptr, raw_cov: Ptr, old_diff: Ptr, new_diff: Ptr,
    weight: Float64, n: Int
):
    for i in range(n):
...
def mpmc_diag_velocity_energy_parallel(
    ...
    return diag_velocity_energy_parallel(
        CtxPtr(unsafe_from_address=ctx)[],
        p(x),
        p(diag),
        p(velocity),
        p(scratch),
...
def mpmc_add_scaled(
    y: Int, x: Int, alpha: Float64, n: Int
) abi("C"):
...
def mpmc_leapfrog_first_diag(
    q: Int, momentum: Int, grad: Int, diag: Int, velocity: Int,
    epsilon: Float64, n: Int
) abi("C"):
    leapfrog_first_diag(p(q), p(momentum), p(grad), p(diag), p(velocity), epsilon, n)
...
def mpmc_leapfrog_first_full(
    q: Int, momentum: Int, grad: Int, matrix: Int, velocity: Int,
    epsilon: Float64, n: Int
) abi("C"):
    leapfrog_first_full(p(q), p(momentum), p(grad), p(matrix), p(velocity), epsilon, n)


@export("mpmc_leapfrog_first_inv")
def mpmc_leapfrog_first_inv(
    q: Int, momentum: Int, grad: Int, chol: Int, velocity: Int,
    epsilon: Float64, n: Int
) abi("C"):
    leapfrog_first_inv(p(q), p(momentum), p(grad), p(chol), p(velocity), epsilon, n)
...
def mpmc_welford_cov_add(
    value: Int, mean: Int, raw_cov: Int, old_diff: Int, new_diff: Int,
    weight: Float64, n: Int
) abi("C"):
    welford_cov_add(
